package com.lxc.partnerMatch.job;
...
//		锁对象：通常并不直接存储对象数据，而是通过控制对某个资源或代码段的访问权限来保证并发安全性。
		RLock lock = redissonClient.getLock(DistributedLockKeys.PARTNER_MATCH_CACHE_LOCK_KEY);
		try {
			...
			if (lock.tryLock(0, 30000L, TimeUnit.MILLISECONDS)) {
				...
				for (Long userId : mainUserList) {
					...
					String redisKey = String.format(RedisKeys.LOGIN_USERID_LOCK, userId);
